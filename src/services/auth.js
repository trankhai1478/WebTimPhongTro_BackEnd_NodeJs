...
export const resgisterService = ({ phone, password, name }) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            error: token ? 0 : 2,
            msg: token ? 'Register is succeed' : 'Phone number has been aldready used !',
            token: token || null
        })
    } catch (error) {
        reject(error);
...
export const loginService = ({ phone, password }) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            error: token ? 0 : 2,
            msg: token ? 'Register is succeed' : response ? 'Password is wrong !' : 'Phone number not found!',
            token: token || null
        })
    } catch (error) {
        reject(error);
